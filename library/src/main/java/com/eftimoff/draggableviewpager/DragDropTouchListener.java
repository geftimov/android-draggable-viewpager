...
public abstract class DragDropTouchListener implements RecyclerView.OnItemTouchListener {
    ...
    private int mobileViewStartX = -1;
    ...
    public DragDropTouchListener(RecyclerView recyclerView, Activity activity, final int page) {
    ...
    public DragDropTouchListener(RecyclerView recyclerView, Activity activity, final int page, Drawable dragHighlight) {
        this(recyclerView, activity, page);
    ...
    public void startDrag() {
        ...
        mobileViewStartX = viewRawCoords[0];
...
//		mobileView.bringToFront();
    ...
    private boolean move(MotionEvent event) {
        ...
        changePageIfNeeded(currentX);


        int deltaY = currentY - downY;
        int mobileViewY = mobileViewStartY + deltaY;
        int deltaX = currentX - downX;
        int mobileViewX = mobileViewStartX + deltaX;
        mobileView.setY(mobileViewY);
        mobileView.setX(mobileViewX);
    ...
    private void changePageIfNeeded(int currentX) {
        final int maximumOffset = displayMetrics.widthPixels / 10;
        //the view is in the left 10% of the screen
        if (currentX <= maximumOffset) {
            final int toPage = page - 1;
            swipeToPage(toPage);
            return;
        }
        //the view is in the right 10% of the screen
        if (displayMetrics.widthPixels - currentX <= maximumOffset) {
            final int toPage = page + 1;
            swipeToPage(toPage);
        }
    }
    ...
    protected abstract void swipeToPage(final int page);


    protected abstract void startSwiping();

    protected abstract void stopSwiping();
